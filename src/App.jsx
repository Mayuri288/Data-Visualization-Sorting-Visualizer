...
import DistributionVisualizer from './Components/distributionVisualizer/distributionVisualizer'
...
async function loadDataset() {
  const response = await fetch("/2019.csv");
...
function reducer(state, { type, payload }) {
  ...
  // get the parent node and the child node to remove
  const parent = d3.select('.canvas-container').node();
  const child = d3.select('.line-container').node();
  
  // check if the parent node contains the child node before removing it
...
function App() {
  ...
  useEffect(() => {
    loadDataset().then((data) => {
      setDataset(data);
      const newValues = d3.shuffle(data.slice(0, initialState.n));
      dispatch({ type: ACTIONS.CHANGE_N, payload: { ...initialState, n: initialState.n, setSortedArray, canvas, newValues, setButtonState, setCanvas } });
    });
  }, []);
      ...
      <Routes>
        <Route path="/" element={
          <SortPage {...{sortData: sortData[0], ...sortPageObj}} innerRef={sortPageRef}>
            ...
            <Visualizations data={state.values} />
          </SortPage>
        }/>
